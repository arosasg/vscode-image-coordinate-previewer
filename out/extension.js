...
function activate(context) {
    ...
    // Register command
    let disposable = vscode.commands.registerCommand('imageCoordinatePreviewer.openImage', (uri) => {
        if (uri) {
    ...
    const providerRegistration = vscode.window.registerCustomEditorProvider('imageCoordinatePreviewer.imagePreview', provider);
    context.subscriptions.push(disposable, providerRegistration);
}
exports.activate = activate;
function openImagePreview(context, uri) {
    const panel = vscode.window.createWebviewPanel('imageCoordinatePreviewer', `Image Coordinates - ${path.basename(uri.fsPath)}`, vscode.ViewColumn.One, {
        enableScripts: true,
...
class ImageCoordinateEditorProvider {
    constructor(context) {
